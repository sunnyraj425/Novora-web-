...
export const CustomerAuth: React.FC<CustomerAuthProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (showForgotPassword) {
      setLoading(true);
      try {
        const res = await sendCustomerPasswordReset(cleanEmail);
        if (res.success) {
    ...
    if (mode === 'signup') {
      if (!name.trim()) {
        setFormError('Please enter your full name for your customer account.');
        return;
      }
    ...
    try {
      if (mode === 'login') {
        const res = await signInCustomerWithEmail(cleanEmail, password);
        if (res.success) {
          onSuccess();
        } else {
        ...
        const res = await signUpCustomerWithEmail(cleanEmail, password, name.trim());
        if (res.success) {
          onSuccess();
        } else {
  ...
  const handleDirectPasswordReset = async () => {
    if (!email.trim() || !validateEmail(email)) {
      ...
      setFormError('Please enter your email address to receive the password reset link.');
      return;
    }
    setLoading(true);
    setFormError(null);
    try {
      const res = await sendCustomerPasswordReset(email.trim());
      if (res.success) {
        setResetSuccessMessage('Password reset email sent. Please check your inbox.');
        setShowForgotPassword(true);
      } else {
        ...
        <div className="w-full max-w-md bg-[#080808] border border-[#D4AF37]/25 rounded-2xl p-6 sm:p-8 shadow-[0_25px_60px_rgba(0,0,0,0.9)] space-y-6">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-xs text-[#888] leading-relaxed">
              {showForgotPassword 
                ? 'Enter your registered email to receive a password reset link. If you previously registered using Google, this will safely set an email password without creating a duplicate account.'
                : mode === 'login' 
          ...
          {/* Tab Switcher: Login vs Sign Up (hidden in Forgot Password mode) */}
          {!showForgotPassword && (
          ...
          {/* Google Fast Sign In (available on Login and Signup) */}
          {!showForgotPassword && (
            <div>
              <button
                type="button"
          ...
          {/* Success Banner (e.g. Password Reset Confirmation) */}
          {resetSuccessMessage && (
            <div className="p-3.5 rounded-lg bg-emerald-950/40 border border-emerald-500/40 text-emerald-200 text-xs flex items-start gap-2.5">
              <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
              <div className="space-y-1">
                <span className="font-medium text-emerald-100 block">{resetSuccessMessage}</span>
                <span className="text-[11px] text-emerald-300/80 block">
                  Click the link in the email to set your new password, then return here to sign in.
                </span>
              </div>
            </div>
          )}
          ...
          {/* Error Banner */}
          {(formError || authError) && (
            <div className="p-3.5 rounded-lg bg-red-950/40 border border-red-500/40 text-red-200 text-xs flex flex-col gap-2">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="text-center pt-2">
                <button
                  ...
                  className="text-xs text-[#A8A8A8] hover:text-[#D4AF37] transition-colors cursor-pointer font-medium"
                >
                  ← Return to Sign In
                </button>
              </div>
                ...
                <div className="flex items-center justify-between mb-1">
                  ...
                  {mode === 'login' ? (
                    <button
                      ...
                      className="text-[11px] text-[#D4AF37] hover:text-[#F5D76E] transition-colors cursor-pointer font-medium"
                    >
                      Forgot Password?
                    </button>
                  ) : (
                    <span className="text-[10px] text-[#A8A8A8]">Min. 6 chars</span>
                  )}
                </div>
                <div className="relative">
          ...
          <div className="pt-2 border-t border-white/5 flex items-center justify-between text-[11px] text-[#777]">
            ...
            <button
              ...
              className="text-[#888] hover:text-[#D4AF37] transition-colors underline font-mono text-[10px] cursor-pointer"
            >
              Admin Portal →
            </button>
          </div>
